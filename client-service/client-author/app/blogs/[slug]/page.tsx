...
async function BlogDetailContent({ slug, lang }: { slug: string; lang: string }) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 py-16">
        <div className="mx-auto max-w-4xl px-4 text-center">
          ...
          <Link href={`/blogs?lang=${lang}`} className="mt-4 text-blue-600 hover:underline">
            ← Back to Blogs
          </Link>
        </div>
      </div>
  ...
  return (
    <article className="min-h-screen bg-slate-50">
      ...
      >
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <span className="text-sm text-slate-500">{blog.readTime} min read</span>
          </motion.div>
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">
                {new Date(blog.publishedAt).toLocaleDateString(
                  lang === "vi" ? "vi-VN" : lang === "sv" ? "sv-SE" : "en-US",
                )}
              </p>
            </div>
          </motion.div>
      ...
      >
        <div className="prose prose-slate max-w-none">
          ...
          <div className="mt-12 rounded-lg border border-blue-200 bg-blue-50 p-8">
            ...
            <ul className="mt-4 space-y-2 text-slate-700">
              <li>• Comprehensive strategies based on proven best practices</li>
              <li>• Practical insights for immediate implementation</li>
              <li>• Real-world examples and case studies</li>
              <li>• Performance metrics and measurable outcomes</li>
            </ul>
          </div>

          {/* Tags */}
          <div className="mt-12 border-t border-slate-200 pt-8">
            <p className="mb-4 text-sm font-medium text-slate-700">Tags:</p>
        ...
        >
          <h3 className="text-xl font-bold text-slate-900">Ready to transform your storefront?</h3>
          <p className="mt-2 text-slate-600">Explore our templates and products to get started.</p>
          <Link
            href={`/?lang=${lang}`}
...
export default async function BlogDetailPage({ params }: BlogDetailProps) {
  ...
  const searchParams = new URL(`http://localhost?${typeof window === "undefined" ? "" : "lang=en"}`)
    .searchParams;
  ...
  return (
    ...
    >
      <BlogDetailContent slug={slug} lang={lang} />
    </Suspense>
  );
}
