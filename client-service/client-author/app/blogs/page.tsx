...
import { translations, resolveLocale } from "@/lib/translations";
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      >
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          ...
          >
          Explore articles on design, e-commerce, and digital experiences
          </motion.p>
        </div>
      </motion.section>
      ...
      >
        <div className="mb-10 flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:p-5">
          ...
          >
            <input
              ...
              placeholder={`Search ${t.blogs.toLowerCase()} by name...`}
              ...
              aria-label="Search blogs by name"
            />
            <button
              type="submit"
          ...
          <select
            ...
            onChange={(event) => updateFilters({ q: searchParams.get("q") ?? "", category: event.target.value })}
        ...
        {filteredBlogs.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
            <h2 className="text-xl font-semibold text-slate-900">No blogs found</h2>
            <p className="mt-2 text-sm text-slate-500">Try another name or topic.</p>
          </div>
        ) : (
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {filteredBlogs.map((blog) => (
            <motion.div key={blog.slug} variants={itemVariants}>
              <Link href={`/blogs/${blog.slug}?lang=${lang}`} className="group block h-full">
    ...
    <Suspense
      fallback={
        <div className="min-h-screen bg-slate-50 py-16">
          <div className="mx-auto max-w-6xl px-4 text-center text-slate-500">Loading blogs...</div>
        </div>
      }
    >
      <BlogsContent />
